Page({
  data: {
    item:{
    questions: ['下面关于系统调用的描述中,错误的是( )', 'Linux下两个进程可以同时打开同一个文件,这时如下描述错误的是( )', '下述是Linux下多线程编程常用的pthread库提供的函数名和意义,说法错误的是?', '在CPU和物理内存之间进行地址转换时,( )将地址从虚拟（逻辑）地址空间映射到物理地址空间', '下面说法正确的是( )', '下列程序代码在Linux系统执行后"*"会被输出多少次( )\nint i;\nfor(i=0;i<3;i++){\nfork();\nprintf("*\\n");\n}','下列哪些因素不会限制Linux服务器并发连接数?( )'],

    questionsA: ['A.系统调用把应用程序的请求传输给系统内核执行', 'A.两个进程中分别产生生成两个独立的fd', 'A.pthread_create创建一个线程', 'A.TCB', 'A.各个线程拥有相同的堆栈', 'A.14','A.系统内存大小'],

    questionsB: ['B.系统调用中被调用的过程运行在"用户态"中', 'B.两个进程可以任意对文件进行读写操作，操作系统并不保证写的原子性', 'B.pthread_join调用一个线程', 'B.MMU', 'B.i++是线程安全的,不需要同步', 'B.16','B.系统网卡数量'],

    questionsC: ['C.利用系统调用能够得到操作系统提供的多种服务', 'C.进程可以通过系统调用对文件加锁，从而实现对文件内容的保护', 'C.pthread_mutex_init初始化一个线程互斥锁', 'C.CACHE', 'C.x=y是线程安全的,不需要同步', 'C.30','C.系统最大文件句柄数量'],

    questionsD: ['D.系统调用保护了一些只能在内核模式执行的操作指令', 'D.任何一个进程删除该文件时，另外一个进程会立即出现读写失败', 'D.pthread_exit结束一个线程', 'D.DMA', 'D.对进程来说,其虚拟内存的大小不受物理内存大小的限制', 'D.32','D.系统IP地址数量'],

    answer: ['B','D','B','B','D','A','B'],
